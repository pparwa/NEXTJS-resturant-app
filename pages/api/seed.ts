...
export default async function handler(
  ...
  await prisma.cusins.deleteMany();
  await prisma.review.deleteMany()
  ...
  await prisma.cusins.createMany({
    data: [{ name: "indian" }, { name: "italian" }, { name: "mexican" }],
  });

  const locations = await prisma.location.findMany();
  const cuisines = await prisma.cusins.findMany();
  ...
  await prisma.resturant.createMany({
    data: [
      // INDIAN //
      {
      ...
      },
      {
        ...
        price: PRICE.CHEAP,
        description:
          "With 20 years of experience cooking in the finest resturants, our chef is excited to present their vision to you and all our guests. Our caring and committed staff will ensure you have a fantastic experience with us.",
        images: [
        ...
        slug: "ramakrishna-indian-resturant-ottawa",
        location_id: ottawaLocationId,
        cusins_id: indianCuisineId,
      },
      {
      ...
      },
      {
        ...
        price: PRICE.REGULAR,
        description:
          "Welcome to Last Train to Delhi. We are a progressive Indian resturant in the beautiful Glebe community in Ottawa. Our speciality is Northern Indian food, classics like Murg Mahkini and some modern dishes like Crispy Shrimp. We are a small cozy resturant, so make sure that you reserve through OpenTable.",
        images: [
      ...
      },
      {
        ...
        price: PRICE.EXPENSIVE,
        description:
          "At this refined, yet casual, Indian resturant, the portions are large, the wine list is top-notch, and the ambience encourages sharing.",
        images: [
      ...
      },
      {
        ...
        price: PRICE.CHEAP,
        description:
          "This elegant fine dining Indian resturant has been satisfying the Indian tandoori and curry cravings for 12 years in Toronto.",
        images: [
        ...
        open_time: "10:00:00.000Z",
        close_time: "21:00:00.000Z",
        slug: "kamasutra-indian-resturant-and-wine-bar-niagara",
        location_id: niagaraLocationId,
         cusins_id: indianCuisineId,
      },
      // MEXICAN //
      {
        ...
        price: PRICE.REGULAR,
        description:
          "Eldorado Taco resturant is excited to serve you traditional Mexican cuisine, re-imagined with a distinct modern flair, in a stylish setting on Preston street. Striving to bring you some of Ottawa’s best Tacos, margaritas and Tequila. Reserve your table now!",
        images: [
      ...
      },
      {
        ...
        price: PRICE.EXPENSIVE,
        description:
          "The first location in Canada, from famed resturanteurs Noble 33, welcomes patrons into an immersive dining experience.",
        images: [
      ...
      // ITALIAN //
      {
        ...
        close_time: "21:00:00.000Z",
        slug: "cano-resturant-ottawa",
        location_id: ottawaLocationId,
         cusins_id: italianCuisineId,
      },
      {
        name: "Blu Ristorante",
        main_image:
      ...
      },
      {
        ...
        price: PRICE.REGULAR,
        description:
          "Stelvio on Dundas West is an authentic Italian resturant serving classic old world fare using traditional recipes and ingredients. Recipes have been fine-tuned to satisfy the palate of the modern guest, and fresh meals are prepared daily.",
        images: [
      ...
      },
      {
        ...
        price: PRICE.CHEAP,
        description:
          "ēst is a modern, newly reopened resturant serving Italian-French courses, captivating cocktails and wine.",
        images: [
      ...
      },
      {
        ...
        price: PRICE.CHEAP,
        description:
          "Welcome to the newest edition to College street iL PADRINO Ristorante has joined the list of Italian resturants where Chef Connie award winning Italian Chef makes every Italian dish with love like no other. ",
        images: [
  ...
  const vivaanId =
    resturants.find((resturant) => resturant.name === "Vivaan - fine Indian")
      ?.id || 1;
  const RamaKrishnaId =
    resturants.find((resturant) => resturant.name === "RamaKrishna Indian")
      ?.id || 1;
  const coconutLagoonId =
    resturants.find((resturant) => resturant.name === "Coconut Lagoon")
      ?.id || 1;
  const lastTrainToDelhiId =
   resturants.find((resturant) => resturant.name === "Last Train to Delhi")
      ?.id || 1;
  const adrakYorkvilleId =
   resturants.find((resturant) => resturant.name === "Adrak Yorkville")
      ?.id || 1;
  const curryishTavernId =
   resturants.find((resturant) => resturant.name === "Curryish Tavern")
      ?.id || 1;
  const utsavId =
   resturants.find((resturant) => resturant.name === "Utsav")?.id || 1;
  const pukkaId =
    resturants.find((resturant) => resturant.name === "Pukka")?.id || 1;
  const kamasutraIndianId =
    resturants.find((resturant) => resturant.name === "Kamasutra Indian")
      ?.id || 1;
  const eldoradoTacoId =
   resturants.find((resturant) => resturant.name === "Eldorado Taco")?.id ||
    1;
  const laBartolaId =
   resturants.find((resturant) => resturant.name === "La Bartola")?.id || 1;
  const elCatrinId =
    resturants.find((resturant) => resturant.name === "El Catrin")?.id || 1;
  const mariachisId =
    resturants.find((resturant) => resturant.name === "3 Mariachis")?.id ||
    1;
  const canoresturantId =
   resturants.find((resturant) => resturant.name === "Cano resturant")
      ?.id || 1;
  const bluRistoranteId =
   resturants.find((resturant) => resturant.name === "Blu Ristorante")
      ?.id || 1;
  const stelvioId =
    resturants.find((resturant) => resturant.name === "Stelvio")?.id || 1;
  const sofiaId =
  resturants.find((resturant) => resturant.name === "Sofia")?.id || 1;
  ...
  await prisma.item.createMany({
    data: [
      ...
      },
      {
        ...
        price: "$12.00",
        resturant_id: lastTrainToDelhiId,
      },
      {
      ...
      },
      {
        ...
        price: "$8.99",
        resturant_id: kamasutraIndianId,
      },
      {
    ...
    await prisma.review.createMany({
      data: [
        ...
        },
        {
          first_name: "Laith",
          last_name: "Harb",
          text: "resturant was loud and crowded. Food is not worth the price.",
          rating: 3,
          resturant_id: eldoradoTacoId,
          user_id: userLaith.id,
        },
        ...
        },
        {
          first_name: "Josh",
          last_name: "Allen",
          text: "Great food, great staff. You can’t ask for much more from a resturant.",
          rating: 5,
          resturant_id: bluRistoranteId,
          user_id: userJosh.id,
        },
